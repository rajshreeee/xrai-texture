...
def load_swin_model(model_name="swin_large_patch4_window12_384"):
...
class StackedEnsemble(nn.Module):
    def __init__(self):
        super().__init__()
        self.swin1 = load_swin_model()
        self.swin2 = load_swin_model()
        self.swin_shape = load_swin_model()

        self.fc_pathology = nn.Sequential(
            nn.Linear(768, 512), nn.ReLU(), nn.Dropout(0.5), nn.Linear(512, 1)
        )
        self.fc_birads = nn.Sequential(
            nn.Linear(768, 512), nn.ReLU(), nn.Dropout(0.5), nn.Linear(512, 4)
        )
    ...
    def forward(self, x):
        f1 = self.swin1(x)
        f2 = self.swin2(x)
        f_shape = self.swin_shape(x)

        #fused = torch.cat([f1, f2], dim=1)

        out_pathology = self.fc_pathology(f1)
        out_birads = self.fc_birads(f2)
...
os.makedirs("tuned-stacked-ensemble-plots-all-two-swins-no-shuffle", exist_ok=True)
...
plt.tight_layout()
plt.savefig("tuned-stacked-ensemble-plots-all-two-swins-no-shuffle/training_curves.png")
plt.close()

print("[INFO] Training plots saved to tuned-stacked-ensemble-plots-all-two-swins-no-shuffle/training_curves.png")

torch.save(model.state_dict(), "tuned-stacked-ensemble-plots-all-two-swins-no-shuffle.pth")
print("[INFO] Model saved as tuned-stacked-ensemble-plots-all-two-swins-no-shuffle.pth")
