...
# 5 texture kernels from feature-ranking analysis (same as run3-ft)
kernels = [
    np.array([
...
def main():
    conditions = [
        # ("A_baseline",      None,            0),
        # ("B_enc2_init",     ["enc2"],        0),
        # ("C_enc2_freeze5",  ["enc2"],        5),
        ("D_enc2_freeze10",  ["enc2"],        10),   # kept as comparison anchor

        ("E_enc1_init",      ["enc1"],         0),
        ("F_enc1_freeze5",   ["enc1"],         5),
        ("G_enc1_freeze10",  ["enc1"],        10),
        ("H_enc12_init",     ["enc1", "enc2"], 0),
        ("I_enc12_freeze5",  ["enc1", "enc2"], 5),
        ("J_enc12_freeze10", ["enc1", "enc2"],10),
    ]
    ...
    for group_name, inject_blocks, freeze_epochs in conditions:
        print(f"\n{'='*50}")
        print(f"Running group: {group_name} | inject_blocks={inject_blocks} | freeze_epochs={freeze_epochs}")
        for seed in SEEDS:
            print(f"  Seed: {seed}")
