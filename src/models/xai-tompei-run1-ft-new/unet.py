...
# based on the actual layer's fan-in.
all_layer_kernels = {
    "layer3": [
        np.array([[-10, -6, -3, -2, -3],
                  [ -5,  0,  2,  3,  2],
...
def resize_kernels_bilinear(kernel_dict, target_size=(3, 3)):
    ...
    """
    resized = {}
    for layer_name, kerns in kernel_dict.items():
        out = []
        for k in kerns:
...
kernels = [k for group in _RESIZED.values() for k in group]  # 5 kernels, 3×3 each
    ...
    if inject_layer is not None:
        if inject_layer == "all":
            for layer_name in ["layer1", "layer2"]:
                inject_kernels(model, layer_name, kernels, seed=KERNEL_JITTER_SEED)
        else:
            inject_kernels(model, inject_layer, kernels, seed=KERNEL_JITTER_SEED)

    return model.to(DEVICE)


def inject_kernels(model, layer_name, kernels_list, seed, scale_to_kaiming=True):
    """
    Inject texture kernels into encoder.{layer_name}[0].conv1.
    Each (out_idx, in_idx) slot gets an independently jittered variant:
      - random 90° rotation (0/90/180/270)
      - random sign flip (±1)
    scale_to_kaiming rescales kernels to match He-normal std for the target layer.
    """
    ...
    print(f"Injecting into encoder.{layer_name}[0].conv1 | shape: {tuple(conv_weight.shape)}")
    ...
    with torch.no_grad():
        for out_idx in range(n_out):
            ...
            for in_idx in range(n_in):
                ...
                k = base_k * kaiming_scale
                conv_weight[out_idx, in_idx, :, :] = torch.tensor(k.copy(), dtype=torch.float32)
...
def tversky_loss(pred, target, alpha=0.3, beta=0.7, eps=1e-6):
    """alpha: FN weight, beta: FP weight (set high to punish blob predictions)."""
...
def train_one_run(group_name, seed, inject_layer, train_loader, val_loader, test_loader, freeze_epochs=0):
    model = build_model(seed, inject_layer)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=5e-4)
    scheduler = CosineAnnealingLR(optimizer, T_max=EC_EPOCHS, eta_min=1e-7)

    if inject_layer is not None and freeze_epochs > 0:
        target = getattr(model.encoder, inject_layer)
        for p in target[0].conv1.parameters():
            p.requires_grad = False

    logger = setup_logger(group_name, seed)
    logger.info(f"Starting | group={group_name} | seed={seed} | inject_layer={inject_layer} | freeze_epochs={freeze_epochs} | device={DEVICE}")
    ...
    for epoch in range(1, EPOCHS + 1):
        if inject_layer is not None and freeze_epochs > 0 and epoch == freeze_epochs + 1:
            target = getattr(model.encoder, inject_layer)
            for p in target[0].conv1.parameters():
                p.requires_grad = True
            logger.info(f"Unfreezing encoder.{inject_layer}[0].conv1 at epoch {epoch}")
...
def main():
    conditions = [
        ("A_baseline_without_aug",          None,      0), 
        # ("FT_B_layer2_init",    "layer2",  0),
        # ("FT_C_layer2_freeze5", "layer2",  5),
        # ("FT_D_layer1_init",    "layer1",  0),
        # ("FT_E_layer1_freeze5", "layer1",  5),
        # ("B_layer1", "layer1",  0), without jitter
        # ("C_layer1", "layer1",  5),
        ("D_layer1", "layer1",  0), # without aug

    ]

    all_results = []

    for group_name, inject_layer, freeze_epochs in conditions:
        print(f"\n{'='*50}")
