...
# Random kernels: 5 × (3,3), zero-mean unit-std, fixed seed for reproducibility.
# KERNEL_JITTER_SEED matches B_enc1 so rot90/sign-flip pattern is identical;
# only the base kernel content differs from the XAI-extracted kernels.
...
for _i in range(5):
    ...
    if _s > 1e-8:
        _raw[_i] /= _s
KERNELS = list(_raw)   # 5 × (3,3) kernels
...
def train_one_run(group_name, seed, inject_blocks, train_loader, val_loader, test_loader,
    ...
    logger.info("-" * 70)
    logger.info(f"Random kernels (RAND_KERNEL_GEN_SEED={RAND_KERNEL_GEN_SEED}):")
    for i, k in enumerate(KERNELS):
        logger.info(f"  kernel[{i}]: {np.round(k, 4).tolist()}")
    logger.info("-" * 70)
    ...
    run = wandb.init(
        ...
        config={
            ...
            "kernel_type":    "random_gaussian",
            "rand_kernel_gen_seed": RAND_KERNEL_GEN_SEED,
        },
        reinit=True
    )
