...
from shallow_unet import ShallowUNet
import config
...
# Raw 5×5 texture kernels from feature-ranking analysis (same as stdunet run).
all_layer_kernels = {
    "layer3": [
        np.array([[-10, -6, -3, -2, -3],
                  [ -5,  0,  2,  3,  2],
...
KERNELS  = [k for group in _RESIZED.values() for k in group]  # 5 kernels flat
...
def build_model(seed, inject_blocks=None):
    set_all_seeds(seed)
    model = ShallowUNet(in_channels=3, out_channels=1)
...
def inject_kernels(model, block_name, kernels_list, seed, scale_to_kaiming=True):
    ...
    conv_weight = block.conv[0].weight       # (C_out, C_in, 3, 3)
    ...
    kaiming_scale = (kaiming_std / current_std) if (scale_to_kaiming and current_std > 1e-8) else 1.0
...
def train_one_run(group_name, seed, inject_blocks, train_loader, val_loader, test_loader, freeze_epochs=0):
    ...
    logger.info(f"Epochs={EPOCHS} | LR={LR} | BatchSize={BATCH_SIZE} | train_batches={len(train_loader)} | val_batches={len(val_loader)} | test_batches={len(test_loader)}")
    logger.info("-" * 70)
    ...
    logger.info("-" * 70)
    logger.info(f"Done | final val_dice={val_dice:.4f} | final test_dice={test_dice:.4f} | final train_dice={train_dice:.4f}")
    wandb.finish()

    return {"group": group_name, "seed": seed,
            "final_train_dice": train_dice, "final_val_dice": val_dice, "final_test_dice": test_dice}
...
def main():
    ...
    for group_name, inject_blocks, freeze_epochs in conditions:
        print(f"\n{'='*50}")
    ...
    df.to_csv("shallowunet_results.csv", index=False)
