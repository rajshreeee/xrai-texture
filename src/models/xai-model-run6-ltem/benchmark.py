...
def fetch_wandb_runs():
    ...
    for run in runs:
        ...
        # One series per seed
        all_data[group]["train_loss"].append(history["train_loss"].values)
        all_data[group]["train_dice"].append(history["train_dice"].values)
...
def print_early_convergence_table(all_data, key_epochs=[5, 10, 20]):
    ...
    print("\n=== Early Convergence Summary (only till epoch 20!)===")
    print(df.to_string())
    df.to_csv("convergence_summary_epoch20.csv")
    return df


# --- Run everything ---
if __name__ == "__main__":
    all_data = fetch_wandb_runs()
