"""
Kernel Drift Analysis — CBIS-DDSM, VanillaUNet, data-size ablation (kr variant)
Produces:
...
Tracked layers:
  ...
  enc1.conv[3]  — second conv within enc1 block (uninjected)
  enc2.conv[0]  — first conv of enc2 block (uninjected control)

Metrics match kernel-drift-tompei.py:
...
LAYERS_TO_ANALYZE = [
    ...
    ("enc1", 3, "enc1_conv3"),   # 2nd conv in enc1 block; shape (64, 64, 3, 3)
...
# ── Metrics (identical to kernel-drift-tompei.py) ─────────────────────────────
...
def collect_all():
    print("\n" + "=" * 60)
    print("  Collecting drift — CBIS-DDSM, VanillaUNet, data-size (kr)")
    print("=" * 60)
    data = {}
...
def print_c1_comparison(summary):
    """C1: enc1.conv[0] drift — Baseline vs Enc1-L1 across fractions."""
