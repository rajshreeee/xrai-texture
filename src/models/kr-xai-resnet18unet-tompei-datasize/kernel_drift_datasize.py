"""
Kernel Drift Analysis — TOMPEI, ResNet18UNet, data-size ablation (kr variant)
Produces:
...
Baseline checkpoints are loaded from the sibling xai-resnet18unet-tompei-datasize
directory (A2_baseline, frac50 and frac100 only). Missing checkpoints are skipped.
...
BASELINE_CKPT_DIR = Path("/ediss_data/ediss2/xai-texture/src/models/xai-resnet18unet-tompei-datasize/checkpoints")
...
CHECKPOINTS = {
    "Baseline": {"inject_blocks": None,       "ckpt_prefix": "A2_baseline", "ckpt_dir": BASELINE_CKPT_DIR},
    "B_layer1": {"inject_blocks": ["layer1"], "ckpt_prefix": "B_layer1",    "ckpt_dir": CKPT_DIR},
}
...
def compute_drift(condition, fraction_pct):
    ...
    ckpt = cfg["ckpt_dir"] / f"{cfg['ckpt_prefix']}_frac{fraction_pct}_seed{SEED}_best.pt"
...
def collect_all():
    print("\n" + "=" * 60)
    print("  Collecting drift — TOMPEI, ResNet18UNet, data-size (kr)")
    print("=" * 60)
    data = {}
...
def print_c1_comparison(summary):
    """C1: encoder.layer1[0].conv1 drift — Baseline vs B_layer1 (injected layer)."""
