...
def inject_kernels(model, block_name, kernels_list, seed, scale_to_kaiming=True):
    """
    Inject texture kernels into the first Conv2d of an FCN8s encoder block.
    Identical logic to SegNet — enc1.conv[0].weight has the same shape/access pattern.
    """
...
def main():
    ...
    print("\n=== FCN8s Run Summary ===")
    print(summary)
    summary.to_csv("fcn_results.csv")
