...
class JointTransform:
    ...
    def __call__(self, image, mask):
        # 1. Base resize — ensures consistent tensor size
        ...
        if self.augment:
            # 2. Pad + random crop — sole spatial shift mechanism.
            #    Preserves texture direction and frequency content entirely.
            #    Pads 10% on all sides then crops back to original size.
            pad_size = int(0.10 * self.size[0])  # 51px for 512x512
            image = F.pad(image, padding=pad_size, fill=0)
            mask  = F.pad(mask,  padding=pad_size, fill=0)

            padded_h = self.size[0] + pad_size * 2
            padded_w = self.size[1] + pad_size * 2
            i = torch.randint(0, padded_h - self.size[0] + 1, (1,), generator=self.generator).item()
            j = torch.randint(0, padded_w - self.size[1] + 1, (1,), generator=self.generator).item()
            image = F.crop(image, i, j, self.size[0], self.size[1])
            mask  = F.crop(mask,  i, j, self.size[0], self.size[1])

            # 3. Horizontal flip — safe for mammography (bilateral symmetry)
            if torch.rand(1, generator=self.generator).item() > 0.5:
                image = F.hflip(image)
                mask  = F.hflip(mask)

            # 4. Vertical flip — safe, does not alter local texture orientation
            if torch.rand(1, generator=self.generator).item() > 0.5:
                image = F.vflip(image)
                mask  = F.vflip(mask)

            # NOTE: Rotation intentionally excluded — directional texture kernels
            # (Fourier-derived) are orientation-specific; rotating the image creates
            # conflicting gradients that corrupt injected kernel priors.
            # NOTE: Brightness/contrast jitter excluded — kernels were calibrated at
            # specific intensity scales; intensity shifts corrupt texture responses.
