...
LOG_DIR = Path("/ediss_data/ediss2/xai-texture/src/models/xai-resnet18unet-cbis-datasize/logs")
...
filename_pattern = re.compile(r"(A_baseline|B_layer1)_frac(\d+)_seed\d+")
...
for fraction in FRACTIONS:
    ...
    enc  = results.get(fraction, {}).get("B_layer1", {})
    ...
    rows.append({
        ...
        "Layer1 Test Dice": enc_best,
        "Baseline Test IoU": base.get("iou"),
        "Layer1 Test IoU": enc.get("iou"),

        # NEW FAIR COLUMN
        "Target Dice (min best)": target,
        "Baseline Epoch @ Target": base_epoch_to_target,
        "Layer1 Epoch @ Target": enc_epoch_to_target,
    })

df = pd.DataFrame(rows)

if df.empty:
    print("No data found. Parsed results:")
    for frac, models in results.items():
        for m, v in models.items():
            print(f"  frac={frac} model={m} dice={v['dice']} iou={v['iou']} best_epoch={v['best_epoch']}")
    raise SystemExit("No complete fraction rows (need both A_baseline and B_enc1). Check log files and regex patterns.")
