...
def train_one_run(group_name, seed, inject_blocks, train_loader, val_loader, test_loader,
    ...
    if inject_blocks is not None and freeze_epochs > 0:
        for layer_name in inject_blocks:
            for p in getattr(model.encoder, layer_name)[0].conv1.parameters():
    ...
    for epoch in range(1, max_epochs + 1):
        if inject_blocks is not None and freeze_epochs > 0 and epoch == freeze_epochs + 1:
            ...
            logger.info(f"Unfreezing {inject_blocks} conv1 at epoch {epoch}")
...
def main():
    conditions = [
        ("A_baseline", None,        0),
        ("B_layer1",   ["layer1"],  0),  # first ResNet18 encoder block, equivalent to enc1
    ]
